...
var SortablisList = function SortablisList(_ref) {
  ...
  var _onStart = (0, _react.useCallback)(function (event, data, id) {
    ...
    var sortedRows = Object.values(newRowData).sort(function (a, b) {
    ...
    var rd = sortedRows.reduce(function (acc, x) {
      ...
      newRowData[x.id] = _objectSpread({}, newRowData[x.id], {
        style: _objectSpread({}, newRowData[x.id].style, {
          top: runningTop
        })
      });
      runningTop += height;
  ...
  var onDrag = (0, _react.useCallback)(function (event, data) {
    ...
    if (refKeys.length) {
      var _refDimensions$refKey = refDimensions[refKeys[0]],
          firstRowTop = _refDimensions$refKey.top,
      ...
      if (isDraggingUp && dragRowBottom < firstRowTop + firstHeightComparison / 2) {
        newShadow.index = 0;
    ...
    while (orderIndex < ordered.length) {
      var nextId = ordered[orderIndex].id;
      orderIndex += 1;
  ...
  var _onTransitionEnd = (0, _react.useCallback)(function (e, newTransitionId) {
    var currentTransitionId = newTransitionId || transitionId;
    var newIndex = rowData[currentTransitionId].index;

    var newRowData = _objectSpread({}, rowData);

    Object.keys(newRowData).forEach(function (k) {
      var _newRowData$k$style = newRowData[k].style,
          top = _newRowData$k$style.top,
          style = _objectWithoutProperties(_newRowData$k$style, ["top"]);

      newRowData[k] = _objectSpread({}, newRowData[k], {
        style: style
      });
    });
  ...
  var onStop = (0, _react.useCallback)(function (e) {
    var newRowData = _objectSpread({}, rowData);

    var oldTop = newRowData[dragId].style.top;
  ...
  }, [dragId, rowData, shadowStyle, _onTransitionEnd]);
  return _react["default"].createElement("div", _extends({
    className: (0, _clsx["default"])(c.root, classes.root) // eslint-disable-next-line no-return-assign
    ,
    ...
    return _react["default"].createElement(_reactDraggable.DraggableCore, {
      ...
      className: (0, _clsx["default"])(c.row, classes.row, id === dragId && c.row_dragging, dragId !== null && id !== dragId && c.row_notDragging, transitionId !== null && c.row_transitioning),
